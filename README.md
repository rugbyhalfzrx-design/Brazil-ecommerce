...
def load_data():
    df = pd.read_csv(os.path.join(DATA_DIR, "eda_customer.csv"), parse_dates=[
        "order_purchase_timestamp", "order_approved_at",
        "order_delivered_carrier_date", "order_delivered_customer_date",
        "order_estimated_delivery_date"
    ]) #os.path.joinで先ほどos.path.dirnameで作成したファイルパス（DATA＿DIR）とCSVファイルを結合している。　 #parse_datesにより日付で読み込むことができる。
